...
from app.models import Meeting, MeetingEvent, Speaker, SpeakerEmbedding, ActionItem as DBActionItem, Decision as DBDecision, Risk as DBRisk
...
class PersistenceService:
    ...
    async def save_meeting(self, meeting_id: str, title: str = "Live Recording") -> None:
        """Creates a minimal Meeting row so FK constraints on meeting_events are satisfied."""
        existing = await self.db.get(Meeting, meeting_id)
        if existing:
            return
        meeting = Meeting(
            id=meeting_id,
        ...
        try:
            ...
            logger.error(f"Failed to persist meeting row: {e}")
            raise
